...
/**
 * Thrown when the S3P API returns a non-2xx response with a parseable
 ...
 * the partner spec.
 */
public final class S3pApiException extends S3pException {
    ...
    public S3pApiException(int httpStatus, ApiError error, String rawBody) {
    ...
    private static String buildMessage(int httpStatus, ApiError error, String rawBody) {
        if (error != null) {
            return "S3P API error (HTTP " + httpStatus + ", respCode=" + error.respCode()
                    + "): " + error.devMsg();
        }
        return "S3P API error (HTTP " + httpStatus + "): "
